...
# The mode bits are a POSIX guarantee. Windows reports 0o666 for every file it
# has not been told otherwise about, so there is nothing here to assert on it.
...
# `os.geteuid` does not exist off POSIX, and reading it at collection time took
# the whole module down with an AttributeError rather than skipping one test —
# so the suite could not be run at all on a Windows machine. Both halves of the
# condition are the same statement: this test needs a directory the caller
# genuinely cannot write to, and neither root nor Windows can supply one.
@pytest.mark.skipif(
    not hasattr(os, "geteuid") or os.geteuid() == 0,
    reason="root, and Windows, write into unwritable directories",
)
