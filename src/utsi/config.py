...
class Settings:
    ...
    #: Whether a row has to answer `q` to be returned. `terms` wants every word
    #: of the query in the release name; `off` returns whatever the engines
    #: said. On by default because engines lie by omission — a site that cannot
    #: match what it was asked answers with its newest uploads rather than with
    #: nothing, and a fan-out cannot tell that from a search that went well. Off
    #: is for a deployment fronting an index doing its own fuzzy or synonym
    #: matching, where a literal gate would delete the answers it found.
    query_match: str = "terms"
    ...
    def from_env(cls) -> Settings:
        ...
        query_match = _str("QUERY_MATCH", "terms").lower()
        if query_match not in ("terms", "off"):
            raise ValueError(f"{PREFIX}QUERY_MATCH must be 'terms' or 'off'")
