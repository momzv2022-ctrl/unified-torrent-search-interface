...
| Parameter | Notes |
|---|---|
| `q` | Search terms. `.`, `_` and `-` are separators; word order is irrelevant. Every term has to appear in the row — see [Matching](#matching). |
...
TSP's `200` is *matching rows*, and a metasearch cannot take that on trust.
Several public indexes answer a query they cannot match with their newest
uploads rather than with nothing — eztvx.to does it for any `Keywords` it does
not recognise — and by the time those rows reach the merge they are
indistinguishable from real ones. Left alone, `q=micheal+jackson` came back
full of this week's television, `count` and all.

So the merged set is held to the query before it is sorted and paged. **Every
term has to appear in the release name**, in either of the two spellings a
release name uses: inside the name as written (substring, not whole word, so
`巨人` finds `進撃の巨人` — a script with no spaces to tokenise on), or inside
it with the separators closed up, so `spiderman` finds `Spider-Man`. The second
only applies from four characters up, below which closing the gaps starts
welding neighbouring words into matches that were never there. Accents fold and
apostrophes close, so `amelie` finds `Amélie` and `oceans` finds `Ocean's`.

Three rows are kept that a stricter reading would drop, for the same reason the
category filter keeps what it cannot classify — a filter that deletes a correct
answer is worse than one that narrows:

- a term the release name dropped. `The Matrix` is on disk as
  `Matrix.1999.BluRay`, so the fourteen commonest English articles and
  prepositions are tolerated when missing — but only while something else in the
  query still has to match. A search for `the` is a search for `the`.
- a row with no readable name. It cannot be judged, so it is not judged.
- a term one keystroke out. The gate must not be stricter than the engines it
  polices: an index forgiving enough to answer `micheal jackson` with
  `Michael Jackson - Thriller` would have that answer deleted here. A term of
  five characters or more matches a word of the name at edit distance one,
  counting a transposition of neighbours as one edit rather than two — which is
  what `micheal` for `michael` is, and the commonest typo there is.

  **Never for a term carrying a digit.** There the character is the whole
  meaning: `x264` and `x265` are different codecs, `2010` and `2011` different
  years, `S01E01` a different episode. Forgiving one of those would answer a
  question nobody asked, which is this same bug arriving by the other door.

An empty `q` is not gated at all. The generic query that stands in for it is
this server's own invention, and holding a client's "browse the whole index" to
a word it never typed would hide rows rather than narrow them.

`UTSI_QUERY_MATCH=off` returns whatever the engines said. That is for a
deployment fronting an `upstream` doing its own fuzzy, stemmed or synonym
matching, where a literal gate here would delete the answers it was asked to
find. On by default, because on the public engines the alternative is answering
