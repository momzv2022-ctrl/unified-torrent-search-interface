...
The program is about 109 KB and compresses to a **66.7 KB** fragment. That is
fine in Chrome and Firefox, which take URLs measured in megabytes. Safari's
ceiling is nearer — around 80,000 characters for the whole URL — so this fits
with roughly 15% to spare.

`worker/tests/worker.test.mjs` fails if a generated link passes 78,000
characters, so growth that would break Safari breaks CI first. If that test ever
goes red, the answer is not to minify the artifact — people paste this into their
own account and some of them read it first — it is to accept that copy-and-paste
becomes the primary route again.
...
**What is still unverified is the phone.** A 66 KB fragment has not been opened
