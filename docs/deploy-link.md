...
> **Status 2026-08-18: neither link deploys.** The payload still loads in the
> playground, but *Deploy* fails from there and from the direct link. Use
> copy-and-paste until this is resolved. Details under
> [Skipping the editor](#skipping-the-editor).
...
> **Both link routes are broken upstream as of 2026-08-18.**
>
> The direct deploy screen answers `Incoming deployment from Workers Playground
> missing worker hash` and renders "The Cloudflare Dashboard is temporarily
> unavailable" instead of the deploy form. Reproduced in Chrome with the fragment
> present and absent: with no fragment the dashboard boots and reports a missing
> payload, with one it hits its error boundary. Not a length problem and not an
> outage — Cloudflare expects a worker hash this link does not carry, exactly the
> undocumented-format drift warned about above.
>
> The **playground** route still *loads* correctly — the program arrives in the
> editor and answers in the preview, confirmed the same day — but the playground's
> own *Deploy* button then fails too, which lands on the same screen. So loading
> the payload works and deploying it does not, by either path.
>
> **Copy-and-paste is therefore the only working route to a deployed Worker.**
> The setup page is deliberately left leading with the deploy link rather than
> shuffled between two dead buttons; reordering them was tried on 2026-08-18 and
> reverted, because it moved people from one broken route to another. The real
> fix is either Cloudflare restoring the format or the page promoting
> copy-and-paste to the primary route — see the checklist below.
