...
## The query gate, and why it is off here

Both runners set `query_match` to `off` unless a scenario names it. The record
above predates the gate, and these fixtures answer every query with the same
canned rows — `knaben.json` returns Sintel however it is asked. Leaving the gate
at its shipped default would rewrite fifteen of the frozen answers for a reason
that has nothing to do with the plumbing they exist to pin, and the record would
be gone. It is off, the original outputs are byte-identical, and the six
`query-gate-*` scenarios at the end turn it on by name.
